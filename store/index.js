...
export default {
  state() {
    return {
      ...
      config_aplikasi: {
        id: 'CONF01',
        akronim_nama_aplikasi: 'ETHOL',
        nama_aplikasi: 'Enterprise Hybird Online Learning',
        semester_aktif: null,
        success: false,
        tahun_ajaran_aktif: '',
        tahun_aktif: null,
  ...
  mutations: {
    ...
    ISI_CONFIG(state, payload) {
      state.config_aplikasi = payload
    },
  ...
  getters: {
    ...
    semester(state) {
      return state.config_aplikasi.semester_aktif
    },
    tahun(state) {
      return state.config_aplikasi.tahun_aktif
    },
    tahun_ajaran(state) {
      return state.config_aplikasi.tahun_ajaran_aktif
    },
    admin(state) {
  ...
  actions: {
          ...
          if (!state.user.id) {
            try {
              ...
              if (Object.prototype.hasOwnProperty.call(user, 'nomor')) {
                ...
                const configAplikasi = await rp({
                  method: 'GET',
                  uri: 'https://ethol.pens.ac.id/api/auth/config',
                  json: true,
                  proxy: false,
                  headers: {
                    token,
                  ...
                  .then((responsAplikasi) => responsAplikasi)
                  .catch(function (err) {
                    console.log('configAplikasi', err)
                    return null
                  })
                commit('ISI_CONFIG', configAplikasi)
                if (hakAktif) {
    ...
    isiConfig({ commit }, payload) {
      commit('ISI_CONFIG', payload)
    },
  },
  modules: { user },
}
