<template>
  <v-container>
    ...
    <v-card flat>
      <v-card-title> Server Converence </v-card-title>
      <v-divider></v-divider>
      <v-card-text>
        <v-row>
...
import Dialog from '@/components/master/server-conference/dialog.vue'
export default {
  components: {
    Breadcrumb,
    'dialog-form': Dialog,
  ...
  methods: {
    ...
    hapusData() {
      ...
      this.$axios
        .delete(`/server-conference/${this.itemData.nomor}`)
        .then((res) => {
          if (res.data.sukses) {
            this.dialogHapus = false
