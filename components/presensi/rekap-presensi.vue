<template>
  <v-card outlined>
    <v-card-title> Data Rekap Presensi </v-card-title>
    <v-divider></v-divider>
    <v-card-text>
      <v-row>
  ...
  methods: {
    ...
    ambilData() {
      ...
      if (
        ...
        const urlApi = '/presensi/' + this.urlAkses
        this.$axios
          .get(urlApi, {
            params: {
              tahun: this.tahun,
    ...
    detailPresensi(v) {
      ...
      this.$emit('detail', Object.assign(v, filterPresensi))
    },
    exportExcel() {
      const daftarExport = [
      ...
      XLSX.writeFile(wb, `Rekap-Presensi-${this.bulanIndo}-${this.tahun}.xlsx`)
    },
  },
}
</script>
