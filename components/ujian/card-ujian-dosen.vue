      ...
      <v-row>
        <v-col cols="10">
          <div class="container-kuliah-mobile">
            ...
            <div>
              <span
                v-if="ujian.dataKuliahGabungan != 0"
                class="card-subtitle-mobile"
              >
                {{ ujian.kode_kelas + ' Kelas ' + ujian.pararel }}
                <small
                  >dan {{ ujian.dataKuliahGabungan.length - 1 }} kelas
                  lainnya</small
                >
              </span>
            ...
            <div>
              <span class="card-jadwal-mobile">
                {{ ujian.ujian.mulai_indonesia }} s/d
...
export default {
  ...
  methods: {
    keDetail(ujian) {
      this.$store.dispatch('modules/ujian/simpan_ujian_dosen', ujian)
      if (this.jenisUjian == '2') {
        this.$router.push('/dosen/uas/detail')
      } else if (this.jenisUjian == '1') {
        this.$router.push('/dosen/uts/detail')
      } else {
